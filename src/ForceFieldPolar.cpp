...
void ForceFieldPolar::setForceFieldPolarParameters() {
    ...
    setFalseMdPerturb();
    std::cout<<"Enter setForceFieldPolarParameters()"<<std::endl;
    for (int i = 0; i < Topologies.molecules.size(); i++) {
        std::cout<<"topologies.molecules.size()"<<Topologies.molecules.size()<<std::endl;
        for (int j = 0; j < Topologies.molecules[i].moleculeNumber; j++) {
            ...
            for (int k = 0; k < moleculeType.polars.size(); k++) {
               ...
               if(k=0){std::cout<<"moleculeType.polars.size() is:"<<moleculeType.polars.size()<<std::endl;}
               if(k=0){std::cout<<"moleculeType.polars[k].polarizability"<<std::endl;}
               if(k=0){std::cout<<"polarizablity is : "<<moleculeType.polars[0].polarizability<<std::endl;}
...
//    std::cout<< "alpha[0] is "<<ffPolars[0].alpha/1000.0<<"       ""alpha[80] is "<<ffPolars[80].alpha/1000.0<<std::endl;   
    ...
    while ( tol > TOLERANCE && n < iter_max ) {
        ...
        std::cout<<"atom 0 R is "<<R[0]<<std::endl;
        std::cout<<"atom 0 R is "<<R[1]<<std::endl;
        std::cout<<"atom 0 R is "<<R[2]<<std::endl;        
        //start new iteration for pi2[i=1...N][9]
        for (int i = 0; i < DOFn; i++) {
            for (int j = 0; j < DOFn; j++) {
                if (i == j) continue;
...
//This is the perturb MD for OpenMM

void ForceFieldPolar::Pi_tensor_openmm(std::vector<double>& iPi_all, std::vector<double>& iPi, std::vector<Vec3>& R_openmm, const Vec3 (&periodicBoxVectors_openmm)[3]) {
    // calculate the many-body polarizability as a function of configuration using iteration method
    // to iter_max order or already converged. Return actual iteration times.
    double Pi_prev[3][3]={0,0,0,0,0,0,0,0,0};
    double Tij[3][3]={0,0,0,0,0,0,0,0,0};
...
void ForceFieldPolar::Dk_Pi_tensor_openmm(int k, std::vector<double>& iPi_all,
    ...
    //calculate DkTkj (fixed k, j=0,...N-1)
    for (int j = 0; j < DOFn; j++) {
        Dk_Tij_tensor_openmm(k, k, j, DkTij,R_openmm, periodicBoxVectors_openmm);
        for (int a = 0; a < 27 ; a++) DkTkj[27 * j + a] = DkTij[a];
    }
    
    //calculate DkTik (fixed k, i=0,...N-1)
    for (int i = 0; i < DOFn ;i++) {
        Dk_Tij_tensor_openmm(k, i, k, DkTij,R_openmm, periodicBoxVectors_openmm);
        for (int a = 0; a < 27; a++) DkTik[27 * i + a] = DkTij[a];
    }
