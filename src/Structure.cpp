...
Structure::Structure() {
    natoms = 0;
    for (int i = 0; i < 3; ++i)
        boxVectors[i] = OpenMM::Vec3(0, 0, 0);
}

void Structure::loadStructure(const std::string& file) {
...
void Structure::saveStructure(const std::string& file, int step, double time) const {
    if (natoms == 0)
...
void Structure::saveTrajectory(const std::string& file, int step, double time, const std::string& group, int precision) const {
    if (natoms == 0)
...
void Structure::getBoxVectors(OpenMM::Vec3& a, OpenMM::Vec3& b, OpenMM::Vec3& c) const {
    a = this->boxVectors[0];
    b = this->boxVectors[1];
    c = this->boxVectors[2];
}

void Structure::setNumAtoms(int natoms) {
    if (this->natoms != 0)
...
void Structure::setAtomInfo(const std::vector<std::string>& atominfo) {
...
void Structure::setVelocities(const std::vector<OpenMM::Vec3>& velocities) {
...
void Structure::setForces(const std::vector<OpenMM::Vec3>& forces) {
...
void Structure::setBoxVectors(const OpenMM::Vec3& a, const OpenMM::Vec3& b, const OpenMM::Vec3& c) {
...
void Structure::checkBoxVectors(const OpenMM::Vec3& a, const OpenMM::Vec3& b, const OpenMM::Vec3& c) const {
    if (a[1] != 0.0 || a[2] != 0.0)
...
void Structure::readGROFile(const std::string& file) {
    ...
    for (std::string line; getline(grofile, line); ) {
	  ...
	  //  std::cout<<"load position : "<< px << "  "<<py<< "  "<<pz<<std::endl;
            ...
            if (hasVel) {
                ...
                velocities[lineNumber-3] = OpenMM::Vec3(vx, vy, vz);
            }
        }
        ...
        // rectangular box with 3 values, or a triclinic box with 9 values.
            ...
            if (values.size() == 3) {
                a = OpenMM::Vec3(values[0], 0.0, 0.0);
                b = OpenMM::Vec3(0.0, values[1], 0.0);
                c = OpenMM::Vec3(0.0, 0.0, values[2]);
            }
            // Perhaps it is a triclinic box if the last 6 values are not zero.
            else if (values.size() == 9) {
                a = OpenMM::Vec3(values[0], values[3], values[4]);
                b = OpenMM::Vec3(values[5], values[1], values[6]);
                c = OpenMM::Vec3(values[7], values[8], values[2]);
...
void Structure::writeGROFile(const std::string& file, int step, double time, const std::vector<int>& indices, bool isTraj) const {
    if (GetFileSuffix(file) != "gro")
...
void Structure::writeXTCFile(const std::string& file, int step, double time, const std::vector<int>& indices, int precision) const {
...
// TODO: not finished
void Structure::readTRRFile(const std::string& file, int step, double time) {
...
void Structure::writeTRRFile(const std::string& file, int step, double time, const std::vector<int>& indices) const {
    if (GetFileSuffix(file) != "trr")
